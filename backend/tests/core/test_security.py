...
def test_create_access_token_with_custom_expires_delta() -> None:
    ...
    # Verify expiration time is approximately 2 hours from now (with tolerance)
    exp_time = datetime.fromtimestamp(payload["exp"], tz=UTC)
    now = datetime.now(UTC)
    expected_time = now + custom_expiry
...
def test_create_access_token_default_expiration() -> None:
    ...
    exp_time = datetime.fromtimestamp(payload["exp"], tz=UTC)
    now = datetime.now(UTC)
    expected_time = now + default_expiry
    # Allow 1 second tolerance for execution time
    assert abs((exp_time - expected_time).total_seconds()) < 1
